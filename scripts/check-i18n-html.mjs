...
/**
 * Pages this check does not apply to.
 *
 * Two different reasons, deliberately in one list because both are "this page
 * is not part of the translated application":
 *
 *   - **Operator screens.** English on purpose — see check-i18n.mjs for why.
 *   - **Single-market campaign pages.** `lancement.html` is French throughout
 *     and loads no catalogue at all. Wiring its prose to translation keys would
 *     mean inventing English and Spanish copy for a page that is never served
 *     in either, and the keys would drift the moment the campaign copy changed.
 *     It is exempt from the language switcher in check-site.mjs for the same
 *     reason, and named there too.
 *
 * A page earns a place here by decision. Forgetting to translate one is still
 * a build failure everywhere else.
 */
